...
The route accepts multipart `image`, optional `model`, optional bounded `guidance`, explicit `dataset_guidance=0|1`, and the current required `contribution_terms` version. Guidance is normalized to one line, capped at 600 characters, and may affect emphasis or formatting only. BetterDiscord defaults to the verified `llamacpp::heretic-8b-q8_0` model while the safe model endpoint publishes every currently verified multimodal choice. The selected Qwen/Gemma Vision model observes the image and writes the final KREA2 prompt itself; there is no separate legacy text-composer pass. A local Heretic run keeps one isolated llama.cpp child for its selected analysis profile, records peak VRAM, and releases or briefly retains the child under the exact shared FIFO rules.
