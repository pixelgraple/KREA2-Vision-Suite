...
let parser = fs.readFileSync(parserPath, "utf8")
    ...
    .replace(
        /\nmodule\.exports = Object\.freeze\(\{\s*DEFAULT_LIMITS,\s*parsePngPromptMetadata\s*\}\);\s*$/,
        "\nreturn Object.freeze({DEFAULT_LIMITS, parsePngPromptMetadata});\n"
    );

if (!/return Object\.freeze\(\{DEFAULT_LIMITS, parsePngPromptMetadata\}\);/.test(parser)) {
    throw new Error("Could not transform the parser export into an inline factory return.");
}

const externalParserBlock = /let parseHardenedPngPromptMetadata = null;\s*try \{[\s\S]*?\n\}\s*catch \{[\s\S]*?\n\}/;
