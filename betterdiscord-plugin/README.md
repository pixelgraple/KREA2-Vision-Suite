...
The Krea2 Vision header and every usable prompt result also expose **Qwen Prompt Editor**. Its **Edit existing prompt** tab revises a complete KREA2 prompt through ordinary language. Its new **Text to prompt** tab accepts a few sentences describing the desired image, expands them into a complete detailed KREA2-ready prompt, and places the result directly into the prompt box for copying or follow-up refinement. Successful output costs one Online API credit per started 350 output tokens; opening, closing, typing, copying, browsing history, and starting a new chat cost nothing, while failed work is refunded automatically. Closing the modal only hides it: running work continues safely, and complete conversations are stored privately on the user's computer across Discord/plugin restarts. Paginated chat/message history can resume any saved session. A permanent 32K-token meter shows the live model window; older model context is summarized and pruned only from inference while full raw history remains local. Conversation content is never stored by the KREA2 gateway. See the complete [Qwen 3.8 Prompt Changer guide](../docs/QWEN_38_PROMPT_CHANGER.md) for detailed workflows, examples, privacy, billing, limits, and troubleshooting.

A separate pinned **Q3.8** icon sits above Discord's server list and opens general-purpose **Qwen 3.8 Cloud Chat**. Server icons move down one position while this launcher stays locked outside their scroll area. Chats and unfinished drafts persist locally with pagination and the same automatic 32K context compaction. Replies can be copied or downloaded as Markdown; fenced text/code blocks become separately downloadable files. Qwen Chat uses the same one-credit-per-started-350-output-token contract and refund protection as Prompt Editor. See [Qwen 3.8 Discord Chat](../docs/QWEN_38_DISCORD_CHAT.md).
...
Prompt provenance is always visible and honest. The plugin never injects invisible project links, tracking text, settings, or provenance into a generated or copied prompt because hidden text could affect generation and cannot be meaningfully reviewed by the user.
