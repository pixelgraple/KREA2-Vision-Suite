...
KREA2 Vision Suite adds an image-description tool to Discord. Install BetterDiscord, install this plugin, choose which Discord servers may use it, and then click the magnifier in the top-right corner of an image. KREA2 Vision examines that image and gives you one detailed reconstruction prompt by default, with three V2 variations available as an option.
...
The **Online API** is an optional alternative for users who do not want to run a large Vision model on their own GPU. Local GPU mode remains account-free. On first Online API use, the plugin opens Discord's OAuth sign-in with only the `identify` scope; after approval, the browser returns the result to the plugin automatically. The KREA2 gateway verifies the account with Discord, issues a revocable remote license bound to that account and local installation, and grants 60 introductory credits. Online Vision reserves three credits once per image, regardless of its internal evidence passes; it charges those credits only after the completed image audit and automatically refunds them when the image fails or is cancelled. Qwen Prompt Editor charges one credit per started 350 output tokens and returns unused reserved credits; failed replies are fully refunded. Each account may buy one starter pack of 1,200 credits for $1.50 USD in Bitcoin. After that invoice settles, the starter offer is permanently replaced by regular packs: 2,667 credits for $5, 5,333 for $10, or 10,667 for $20. The plugin never receives a Discord password, BTCPay API key, gateway Discord client secret, or worker credential. For each image, the plugin exchanges its license through a short-lived, request-bound, one-use local session. The local service sends the image over HTTPS to the private KREA2 gateway, which checks both license and available credits before forwarding it to the operator's dedicated RTX 3090. The same private machine serves Gemma 4 26B-A4B Vision and Qwen 3.8 Prompt Editor through a one-model-at-a-time llama.cpp router. Selecting Online API disables local execution for that request; the completed job records the exact remote model that actually ran.

Online inference means the selected image and bounded request metadata must leave the user's PC. The dedicated worker processes request content in memory and does not intentionally write submitted images, generated prompts, or Prompt Editor conversations to worker disk. A shared FIFO serializes Vision, Prompt Editor, and private OpenWebUI calls because the 24 GB GPU holds only one large model at a time; switching workloads unloads the previous model before loading the next. Dataset contribution and rich failure attachments are separate settings with their own disclosures; enabling Online API does not automatically enable either one. Privacy-minimal technical error reporting remains active in every mode and never contains an image, image hash, prompt, Discord identity, URL, filename, or local path.
