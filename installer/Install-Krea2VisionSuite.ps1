...
if($Mode -ne 'PluginOnly'){
    ...
    $modelRoot=Join-Path $installRoot 'models';[IO.Directory]::CreateDirectory($modelRoot)|Out-Null
    Set-EnvValue $visionEnv 'KREA2_DISCORD_VISION_TOKEN' $visionToken;Set-EnvValue $visionEnv 'QWEN_BACKEND' 'llama_cpp';Set-EnvValue $visionEnv 'QWEN_MODEL' $selectedPublicId;Set-EnvValue $visionEnv 'LLAMA_CPP_MODEL_ROOT' $modelRoot;Set-EnvValue $visionEnv 'STUDIO_FORGE_HANDOFF_TOKEN_FILE' $forgeTokenFile
    if(-not $SkipLlamaCppRuntime){Write-Step 'Installing the verified llama.cpp runtime and Vision model';$runtimeInstaller=Join-Path $visionTarget 'scripts\install_heretic_llamacpp.ps1';$runtimeArguments=@('-NoProfile','-ExecutionPolicy','Bypass','-File',$runtimeInstaller,'-InstallRoot',(Join-Path $env:LOCALAPPDATA 'Krea2Vision\llama.cpp\b10590'),'-CacheRoot',(Join-Path $modelRoot '.downloads'),'-ModelRoot',$modelRoot,'-VerifyManualModels');if($Model -ne 'None'){$runtimeArguments+=@('-DownloadModels',$Model)};Invoke-Checked 'powershell.exe' $runtimeArguments}
